...
EMAIL_SUBJECT="IP Address Update"
...
if [[ $OLD_IP != $NEW_IP ]]; then
    ...
    curl --request POST \
      ...
      --data "{\"personalizations\": [{\"to\": [{\"email\": \"${TO_EMAIL_ADDR}\"}]}],\"from\": {\"email\": \"${FROM_EMAIL_ADDR}\", \"name\": \"${FROM_EMAIL_NAME}\"},\"subject\": \"${EMAIL_SUBJECT}\",\"content\": [{\"type\": \"text/plain\", \"value\": \"Your IP has changed: ${OLD_IP} -> ${NEW_IP}\"}]}"
else
